...
serve(async (req) => {
  ...
  try {
    ...
    // Map website URLs using Firecrawl v1 API
    ...
    console.log('Map response:', mapResponse.status, mapText)
    
    if (!mapResponse.ok) {
      throw new Error(`Firecrawl map failed with status ${mapResponse.status}: ${mapText}`)
    }
    
    const mapResult = JSON.parse(mapText)
    
    // Handle both possible response formats
    const urls = mapResult?.links || mapResult?.data?.links || mapResult?.urls || []
    
    if (urls.length === 0) {
      // If map doesn't work, use a fallback list of known URLs
      console.log('Map returned no URLs, using fallback list')
      const fallbackUrls = [
        'https://sidetool.co',
    ...
    // Scrape all URLs
    const scrapePromises = urlsToProcess.map(async (url: string) => {
      try {
        ...
        if (!scrapeResponse.ok) {
          console.error(`Scrape failed for ${url}:`, scrapeText)
    ...
    // If no successful scrapes, try to at least create a basic file
