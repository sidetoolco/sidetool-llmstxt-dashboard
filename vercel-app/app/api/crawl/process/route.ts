...
async function generateFiles(jobId: string, supabase: any) {
  try {
    ...
    // Check if job status indicates files were already generated
    if (job.status === 'completed') {
      console.log(`Job ${jobId} already completed, skipping file generation`)
      return
    }
    ...
    // Force delete any existing files to avoid duplicates
    ...
    if (deleteError) {
      console.log('Error deleting existing files (continuing):', deleteError.message)
    }
  ...
  // Mark job as completed FIRST to prevent concurrent generation
  ...
  if (updateError) {
    console.log('Job status update error (may already be completed):', updateError.message)
  }
  ...
  if (insertError) {
    // If insert fails due to duplicate, files were already created by another process
    if (insertError.code === '23505') {
      console.log('Files already exist (created by another process), skipping')
      return
    }
  ...
  console.log(`Generated ${filesToInsert.length} files for job ${jobId}`)
    ...
    // Don't throw if it's a duplicate error - just log and continue
    if (error.message?.includes('duplicate key')) {
      console.log('Duplicate key error - files were likely generated by another process')
      return
    }
    throw error
  }
