...
import FirecrawlApp from 'https://esm.sh/@mendable/firecrawl-js@0.0.15'
...
serve(async (req) => {
  ...
  try {
    ...
    // Map website URLs
    const mapResult = await firecrawl.mapUrl(baseUrl, { limit: maxUrls })
    ...
    const scrapePromises = urls.map(async (url: string) => {
      try {
        ...
        return {
          ...
          success: !!result?.markdown,
          error: result?.markdown ? undefined : 'No content found'
        }
      } catch (error) {
