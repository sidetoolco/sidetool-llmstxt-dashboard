...
/**
 * LLMs.txt File Generator for Daily Automation
 * This script generates multiple LLMs.txt files for different purposes
 */
...
// Mock blog posts data (in production, this would fetch from your CMS/database)
...
function generateIndividualBlogFiles(posts) {
  const files = {};
...
function generateTopicCollections(posts) {
  const files = {};
...
function generateMainCollectionFiles(posts) {
  const files = {};
...
`;
  ...
  files['llms'] = {
    ...
    size: Buffer.byteLength(llmsTxtContent, 'utf8'),
...
`;
  ...
  files['llms-full'] = {
    ...
    size: Buffer.byteLength(llmsFullContent, 'utf8'),
...
export async function generateMultipleLLMsFiles() {
  console.log('🚀 Generating multiple LLMs.txt files...');
  ...
  try {
    ...
    const totalSize = Object.values(allFiles).reduce((sum, file) => sum + file.size, 0);
...
export { generateIndividualBlogFiles, generateTopicCollections, generateMainCollectionFiles };

// Run if called directly
if (import.meta.url === `file://${process.argv[1]}`) {
  generateMultipleLLMsFiles()
    .then(files => {
      console.log(`🎉 Successfully generated ${Object.keys(files).length} LLMs.txt files`);
    })
    .catch(error => {
